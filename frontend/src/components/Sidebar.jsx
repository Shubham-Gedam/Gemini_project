...
export default function Sidebar({ user, onLogout, setView, setActiveModal }) {
  ...
  return (
    <aside className="w-64 border-r border-zinc-800 flex flex-col p-4 relative bg-zinc-950/20 backdrop-blur-sm">
      <button className="flex items-center gap-2 p-3 rounded-xl border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-900 hover:border-zinc-700 transition-all mb-4 text-sm font-medium">
        <Plus size={18} /> New Chat
      </button>

      <nav className="flex-1 overflow-y-auto space-y-1 text-sm text-zinc-500 scrollbar-hide">
        <div className="flex items-center gap-2 p-2 hover:bg-zinc-900 rounded-lg cursor-pointer transition-colors group">
          <MessageSquare size={16} className="group-hover:text-indigo-400" /> Analysis of data...
        </div>
      </nav>

      {isMenuOpen && (
        <div ref={menuRef} className="absolute bottom-20 left-4 w-56 bg-zinc-950/95 border border-zinc-800 rounded-2xl shadow-2xl z-50 py-2 animate-in fade-in slide-in-from-bottom-2 duration-300 backdrop-blur-xl">
          <div className="space-y-0.5 px-2">
            ...
            {user.isGuest ? (
              ...
              <button 
                onClick={() => { 
                  onLogout(); // Fixed: Calling the logout function
                  setIsMenuOpen(false); 
                }} 
                className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-red-500 hover:bg-red-500/10 rounded-xl transition-all font-semibold"
              >
      ...
      <div className="pt-4 border-t border-zinc-800">
        <div onClick={() => setIsMenuOpen(!isMenuOpen)} className={`group flex items-center gap-3 p-2.5 rounded-2xl transition-all cursor-pointer ${isMenuOpen ? 'bg-zinc-900 ring-1 ring-white/10' : 'hover:bg-zinc-900/50'}`}>
          <div className="relative shrink-0">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-500 flex items-center justify-center text-white shadow-lg group-hover:scale-105 transition-transform"><User size={20} /></div>
